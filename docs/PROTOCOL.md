...
Some control messages (like lobby management) may be exchanged over TCP in specific implementations, but the core gameplay loop typically relies on UDP.
...
| 0x05-0x09   | Lobby & Matchmaking       | Join, leave, lobby state, UDP handshake|
| 0x10-0x1F   | Player Input              | Real-time input commands              |
| 0x20-0x2F   | Room Management           | Custom rooms, room list, host control |
| 0x30-0x3F   | Admin Commands (C→S)      | Admin authentication and control      |
...
| 0xB0-0xBF   | Entity Events             | Spawn, destroy, damage, explosion     |
| 0xC0-0xCF   | Game Mechanics            | Powerups, scoring, waves, levels      |
| 0xD0-0xDF   | Admin Responses (S→C)     | Admin results, notifications          |
| 0xF0-0xFF   | System & Chat (S→C)       | Chat broadcast, system messages       |
...
| 0x08    | CLIENT_UDP_HANDSHAKE  | Associate UDP with TCP session           |
...
Sent via UDP to establish the UDP session associated with the TCP connection.
Size: 8 bytes
- `Player ID` (4 bytes)
- `Session ID` (4 bytes)
...
- `Server Tick` (4 bytes)
- `Level Seed` (4 bytes)
- `UDP Port` (2 bytes) - Port for gameplay communication
- `Map ID` (2 bytes) - Initial map/level ID
...
- `Input Flags` (2 bytes)
- `Client Tick` (4 bytes)
- `Sequence Number` (4 bytes) - Added for lag compensation and reconciliation
...
- `Scroll X` (4 bytes, float) - Global map scroll position
...
- All structures are packed (`__attribute__((packed))`) to prevent padding.
