...
public:
     ...
     * @param callback Function receiving session_id, udp_port, map_id, scroll speed, seed and difficulty
     */
    void set_on_game_start(std::function<void(uint32_t session_id, uint16_t udp_port, uint16_t map_id, float scroll_speed, uint32_t seed, protocol::Difficulty difficulty)> callback);
...
private:
    ...
    std::function<void(uint32_t, uint16_t, uint16_t, float, uint32_t, protocol::Difficulty)> on_game_start_;
