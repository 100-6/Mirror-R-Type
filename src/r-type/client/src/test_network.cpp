...
int main(int argc, char* argv[]) {
    ...
    client.set_on_game_start([&client](uint32_t session_id, uint16_t udp_port, uint16_t map_id, float scroll_speed, uint32_t seed, Difficulty difficulty) {
        std::cout << "[TestClient] GAME START! Session: " << session_id
                  << ", UDP port: " << udp_port << ", Map: " << map_id
                  << ", Scroll speed: " << scroll_speed
                  << ", Seed: " << seed
                  << ", Difficulty: " << difficulty_to_string(difficulty) << "\n";
