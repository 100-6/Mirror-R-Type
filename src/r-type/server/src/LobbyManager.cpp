...
void LobbyManager::update() {
    ...
    for (auto& [lobby_id, lobby] : lobbies_) {
        ...
        else if (countdown_callback_)
            countdown_callback_(lobby_id, static_cast<uint8_t>(remaining));
    }
    for (uint32_t lobby_id : lobbies_to_start) {
        ...
        if (game_start_callback_)
            game_start_callback_(lobby_id, lobby->player_ids);
...
void LobbyManager::notify_lobby_state_changed(uint32_t lobby_id) {
    if (!lobby_state_callback_)
        return;
    auto payload = build_lobby_state_payload(lobby_id);

    if (!payload.empty())
        lobby_state_callback_(lobby_id, payload);
}
